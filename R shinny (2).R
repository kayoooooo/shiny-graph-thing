...
ui<-fluidPage(
  ...
  navlistPanel("Graphical exploration",
               ...
               tabPanel("Tumor Size",
                        ...
                        radioButtons("TumorSort", "Sort by:", choices = c("AMT", "ID", "SEX"), selected = "AMT")
               )),
...
server <- function(input, output,session) {
  ...
  aedata <- read_csv("C:/Users/19736/Downloads/Multiple_Ascending_Dose_Dataset3.csv")
  output$AEPlot = renderPlot( xyplot( tumor ~ TIME, groups = ID, data = aedata[aedata$ID == input$id,], xlab = "Time (hours)", ylab = "Dose Amount", panel = function(...) {
    panel.xyplot(...)
    panel.abline(v = aedata[aedata$ID == input$id,][aedata$AE == "Severe",]$TIME, col = "red")
    panel.abline(v = aedata[aedata$ID == input$id,][aedata$AE == "Mild",]$TIME, col = "yellow")
    panel.abline(v = aedata[aedata$ID == input$id,][aedata$AE == "No",]$TIME, col = "green")
  }))
  
  
  rsdata <- read_csv("C:/Users/19736/Downloads/Multiple_Ascending_Dose_Dataset3.csv")
  output$TumorSizePlot <- renderPlot({
    ggplot(rsdata, aes(x = TIME, y = tumor, color = ID)) + geom_point()+ facet_wrap( ~ get(input$TumorSort), ncol=3)
  })
