...
ui<-fluidPage(
  ...
  navlistPanel("Graphical exploration",
               ...
               tabPanel("Tumor Size",
                        ...
                        radioButtons("TumorSort", "Sort by:", choices = c("NAME","TRTACT","ID","TIME","LIDV","DOSE","AMT1","AE","RESP"), selected = "DOSE")
               ),
...
server <- function(input, output,session) {
  ...
  output$AESlider = renderUI({
    sliderInput(inputId = "id",
                ...
                min = min(aedata()$ID),
                max = 60, #max(dat2$ID),
                value = median(aedata()$ID), #median(output$AEplot$dat2$ID),
                step = 1)
  })
  ...
  output$AESummary = renderDataTable({
    AEdat = aedata()
    AEdat = AEdat[AEdat$NAME == "Dosing" & AEdat$DOSE %in% input$Selected,] %>% group_by(DOSE) %>% summarise(AMT1 = mean(AMT1), "Standard Deviation" = sd(AMT1))
    AEdat
  })
  ...
  output$TumorSizePlot <- renderPlot({
    if (v3$doTable > 0) {
      ...
      ggplot(dat3, aes(x = TIME, y = tumorpc, color = ID)) + geom_smooth(method = "lm") + geom_point()+ facet_wrap( ~ get(input$TumorSort), ncol=3)
    }
  })
