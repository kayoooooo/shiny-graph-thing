...
server <- function(input, output,session) {
  ...
  output$AESummary = renderDataTable({
    ...
    AETable = AEdat[(AEdat$NAME == "Dosing" & AEdat$DOSE %in% input$Selected),] %>% 
      ...
      filter(TIME != 0)
    print(AETable[AETable$Reduction == "Y",])
    AETable = AETable %>%
      group_by(DOSE) %>% 
      summarise("Dose Intensity" = round(geoMean(AMT1, na.rm = TRUE) / DOSE, digits = 4), 
                ...
                "Number of patients without dose reductions" = length(unique(ID[Reduction == "N"])), #length(unique(ID[Reduction == "N"]))
                "Number of patients with 1 dose reduction" = length(unique(ID)), #length(unique(ID[Reduction == "Y"]) )
                "Number of patients with 2 or more dose reductions" = length(unique(ID)), 
                )
    print(AETable)
    AETable
  })
