...
server <- function(input, output,session) {
  ...
  output$TumorSizePlot <- renderPlot({
    if (v3$doTable > 0) {
      ...
      #dat3_means = aggregate(data = dat3, tumorpc ~ NOMTIME, mean)     old method tried
      ...
      #dat3 = merge(dat3, dat3_means, by = "NOMTIME", suffixes = c(".raw",".mean")) #merge for easy implementation
  ...
  output$AESummary = renderDataTable({
    ...
    print(reductTimesTable[reductTimesTable$Reduction == "Y",])
    AETable = AETable %>%
      mutate(reductTimes = ifelse(ID %in% reductTimesTable$ID, reductTimesTable[ID == reductTimesTable$ID,]$n, 0)) #longer object length is not a multiple of shorter object length error here w the TRUE argument in ifelse
    #print(AETable[!is.na(AETable$reductTimes),])
    AETable = AETable %>%
      group_by(DOSE) %>% 
      summarise("Dose Intensity" = round(geoMean(AMT1 / DOSE, na.rm = TRUE), digits = 4), 
                ...
                "Number of patients without dose reductions" = length(unique(ID[reductTimes == 0])),
                "Number of patients with 1 dose reduction" = length(unique(ID[reductTimes == 1])),
                "Number of patients with 2 or more dose reductions" = length(unique(ID[reductTimes > 1]))
      )
    print(AETable)
    AETable
  })
